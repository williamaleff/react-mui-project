...
import { IVFormErrors, useHookFormAgente } from "../../shared/forms";
...
export const DetalheDeAgente: React.FC = () => {
  ...
  useEffect(() => {
    if (id !== 'novo') {
      ...
      AgenteService.getById(Number(id))
      .then((result) => {
          setIsLoading(false);
    ...
    reset({
      ...
      funcaoId: 0,
      telefone: "",
      dataContratacao: todayOfTheTime,
  ...
  const handleSave = (data: TFormData) => {
    formValidationSchema
      ...
      .then((dadosValidados) => {
        ...
        if (id === "novo") {
          AgenteService.create(dadosValidados).then((result) => {
            setIsLoading(false);
            ...
            if (result instanceof Error) {
              ...
              if (isSavingAndClose.current) {
                ...
                navigate(`/agente/detalhe/${result}`);
              }
            }
          });
        } else {
          AgenteService.updateById(Number(id), {
            id: Number(id),
            ...dadosValidados,
          }).then((result) => {
            setIsLoading(false);
    ...
    <LayoutBaseDePagina
      ...
      barraDeFerramentas={
        <FerramentasDeDetalhe
          ...
          aoClicarEmVoltar={() => navigate("/agente")}
          aoClicarEmNovo={() => navigate("/agente/detalhe/novo")}
        />
      }
    >
            ...
            <Grid container item direction="row" spacing={2}>
              <Grid item xs={12} sm={12} md={6} lg={4} xl={2}>
                <TextField
                  ...
                  helperText={
                    errors.dataContratacao ? errors.dataContratacao.message : ""
                  }
                />
              </Grid>
            </Grid>
